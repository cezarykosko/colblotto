...
import blotto.actions.BlottoAgentAction;
import blotto.game.BlottoGameList;
import blotto.predicates.BlottoResult;
...
public abstract class MetaBlottoAgent extends Agent{
	...
	public void handleResponses(Vector responses, Vector acceptances) {
		int index = responses.size()-1 < 3 ? responses.size() - 1 : 3;
			...
			if (response.getPerformative() == ACLMessage.PROPOSE) {
				...
				try {
					if (i == index) {
						...
						ContentElementList cel = this.getContents(msg);
